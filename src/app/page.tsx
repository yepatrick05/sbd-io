...
export default async function Home() {
    // Read the most recently created program so the home page can resume the latest training block.
    ...
    const latestBlock = latestProgram?.blocks[0] ?? null;
    const totalSessionCount = latestProgram === null ? 0 : countSessions(latestProgram.blocks);
    const completedSessionCount =
        latestProgram === null ? 0 : countCompletedSessions(latestProgram.blocks);
        ...
        <main className="space-y-6 p-6">
            <div className="space-y-2">
                ...
                <p className="max-w-xl text-sm text-gray-600">
                    A simple training companion that turns coach spreadsheets into a cleaner session-by-session
                    workflow for lifters.
                </p>
            </div>
            ...
            {latestProgram !== null && (
                <section className="space-y-4 rounded border border-gray-200 bg-white p-4">
                    <div className="space-y-1">
                        <p className="text-sm text-gray-600">Most Recent Program</p>
                        <h2 className="text-xl font-semibold">{latestProgram.name}</h2>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded border border-gray-200 bg-gray-50 p-3 text-sm">
                            ...
                            <p className="font-medium">{formatDate(latestProgram.createdAt)}</p>
                        </div>
                    </div>

                    <div className="flex flex-wrap gap-3 text-sm">
                        <Link
                            href={`/programs/${latestProgram.id}/next`}
                            className="rounded border border-black bg-black px-4 py-2 text-white"
                        >
                            Continue Training
                        </Link>
                        <Link
                            href={`/programs/${latestProgram.id}`}
                            className="rounded border border-gray-300 bg-white px-4 py-2 text-gray-700"
                        >
                            View Program
                        </Link>
